function tabsEvent(event) {
	if (event.target.closest('.tabs__button')) {
		...
		const tabButtons = currentBtn.closest('.tabs').querySelectorAll('.tabs__button');

		const allIds = [];
		tabButtons.forEach(tabBtn => allIds.push(tabBtn.getAttribute('data-tab')));
		const tabsItems = [];
		...
		if (!currentBtn.classList.contains('tabs__button--active')) {
			tabButtons.forEach(tabBtn => tabBtn.classList.remove('tabs__button--active'));
